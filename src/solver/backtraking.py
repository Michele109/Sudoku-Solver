...
def solve_sudoku(grid, stats=None, _depth=0, grid_size=None, block_size=None):
    ...
    empty_cell = find_empty_location(grid, GRID_SIZE=grid_size, BLOCK_SIZE=block_size)
    if not empty_cell:
        if stats is not None:
    ...
    lcv_ordered_nums = get_lcv_ordered_values(
        grid, row, col, GRID_SIZE=grid_size, BLOCK_SIZE=block_size
    )
